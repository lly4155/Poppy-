...
#define kCachedADurl ([[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)objectAtIndex:0]stringByAppendingString:@"/adUrl"])
...
+ (NSString *)getAdUrl
{
    ...
    return [NSString stringWithContentsOfFile:kCachedADurl encoding:NSUTF8StringEncoding error:nil];
}
...
+ (void)loadLatestAdImage
{
    ...
    [[[SXNetworkTools sharedNetworkToolsWithoutBaseUrl]GET:path parameters:nil success:^(NSURLSessionDataTask *task, NSDictionary* responseObject) {
        ...
        NSString *action_params = adArray[0][@"action_params"][@"link_url"];
        if (adArray.count >1) {
             imgUrl2= adArray[1][@"res_url"][0];
            action_params = adArray[1][@"action_params"][@"link_url"];
        }
        //ADurl写入沙盒
        [action_params writeToFile:kCachedADurl atomically:YES encoding:NSUTF8StringEncoding error:nil];

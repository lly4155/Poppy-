...
//获取上层父控制器
-(id)initWithSuperVC:(LyTopicVC *)super{
    self = [super init];
    _super = super;
    return self;
}
...
//    [self.view addSubview:mask];
    ...
    //2.操作提示
    UILabel * tipLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, self.view.sd_height*0.95-kBorderW*2, self.view.bounds.size.width, kBorderW)];
...
//按钮展开动画功能
- (void) onButton:(TBAnimationButton *)sender
{
    if (sender.currentState == TBAnimationButtonStateMenu) {
        [self.ItemsBtn addObjectsFromArray:@[_albumBtn,_flashBtn]];
...
- (void)setupMaskView
{
    ...
    mask.bounds = CGRectMake(0, 0, self.view.sd_width + kBorderW , self.view.sd_width + kBorderW );
...
{
    //1.下边栏
//    UIView *bottomBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.sd_height -60 , self.view.sd_width, 60 )];
//    bottomBar.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
//    
//    [self.view addSubview:bottomBar];
    
    //1.补充遮罩
    UIView *mask=[[UIView alloc]initWithFrame:CGRectMake(0, self.view.sd_height - kBorderW + 38, self.view.sd_width, kBorderW)];
    mask.backgroundColor=[UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
    [self.view addSubview:mask];
    
    //2.我的二维码
    UIButton * myCodeBtn=[UIButton buttonWithType:UIButtonTypeCustom];
    myCodeBtn.frame = CGRectMake(self.view.sd_width/2-30, 10 , 65, 87);
    ...
    [mask addSubview:myCodeBtn];
//    [self.view bringSubviewToFront:myCodeBtn];
...
- (void)setupScanWindowView
{
    ...
    _scanWindow = [[UIView alloc] initWithFrame:CGRectMake(kMargin, kBorderW -2, scanWindowW, scanWindowH)];
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    if (metadataObjects.count>0) {
        ...
        self.super.result.text = metadataObject.stringValue;
...
//        controller.modalTransitionStyle=UIModalTransitionStyleFlipHorizontal;
        [controller.navigationBar setBarTintColor:[UIColor colorWithRed:213/255.0 green:24/255.0 blue:37/255.0 alpha:0.8]];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        if (features.count >=1) {
            ...
            self.super.result.text = feature.messageString;
            [self.navigationController popViewControllerAnimated:YES];
            /*
...
-(void)openFlash:(id)sender{
    if (!self.open) {
//        [self.flashBtn setImage:[UIImage imageNamed:@"qrcode_scan_btn_flash_on"]];
        [self turnTorchOn:YES];
    }
    else{
//        [self.flashBtn setImage:[UIImage imageNamed:@"qrcode_scan_btn_flash_down"]];
        [self turnTorchOn:NO];
    }
    self.open = !self.open;
}
